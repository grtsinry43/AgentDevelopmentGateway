...
test('native rewind apply truncates durable events, items, and the runtime tail', async () => {
  ...
  try {
    ...
    const result = await service.rewind(sessionId, {
      target: { by: 'message', messageUuid: `input-${firstInputSequence}` },
      mode: 'apply'
    })
    ...
    assert.equal(after.lastEventSequence, firstInputSequence)
    const durable = events.listAfter(sessionId, firstInputSequence)
    assert.equal(durable.length, 0)
    const remainingItems = items.listBefore(sessionId, Number.MAX_SAFE_INTEGER, 1000)
    assert.ok(remainingItems.length > 0)
    assert.ok(remainingItems.every((item) => item.sequence <= firstInputSequence))
    assert.ok(
      remainingItems.some(
    ...
    assert.equal(runtime.eventSnapshot(asSessionId(sessionId), firstInputSequence).length, 0)
  } finally {
